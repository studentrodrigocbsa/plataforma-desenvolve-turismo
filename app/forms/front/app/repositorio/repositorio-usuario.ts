...
export class RepositorioUsuario {
    ...
    async cadastro(user: Login){
        const response = await fetch(`${API}/cadastro/usuario`, { method: 'post', body: JSON.stringify(user), headers: {'Content-Type': 'application/json', 'ngrok-skip-browser-warning':'skip it'}, credentials:'include'  } );
